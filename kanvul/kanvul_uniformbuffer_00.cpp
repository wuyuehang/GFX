...
public:
    ~KanVul() {
        ...
        vkFreeMemory(_dev, _uniformBufMem, nullptr);
        vkDestroyBuffer(_dev, _uniformBuf, nullptr);
    ...
    KanVul() {
        ...
        _glfw = glfwCreateWindow(800, 800, "KanVul_uniformbuffer_00", nullptr, nullptr);
    ...
    void InitUniformBuffers() {
        ...
        unibufInfo.size = sizeof(OC);
        unibufInfo.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
        unibufInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
        vkCreateBuffer(_dev, &unibufInfo, nullptr, &_uniformBuf);

        VkMemoryRequirements req {};
        vkGetBufferMemoryRequirements(_dev, _uniformBuf, &req);
        ...
        vkAllocateMemory(_dev, &allocBufInfo, nullptr, &_uniformBufMem);

        uint8_t *pDST = nullptr;
        vkMapMemory(_dev, _uniformBufMem, 0, req.size, 0, (void **)&pDST);

        uint8_t *pSRC = (uint8_t *)((const void *)&OC[0]);
        for (int32_t i = 0; i < req.size; i++) {
            *(pDST + i) = *(pSRC + i);
        }

        vkUnmapMemory(_dev, _uniformBufMem);
        vkBindBufferMemory(_dev, _uniformBuf, _uniformBufMem, 0);
    }
    ...
    void InitGFXPipeline() {
        ...
        descBufferInfo.buffer = _uniformBuf;
...
private:
    ...
    /* resource */
    VkBuffer _vertexBuf;
    VkBuffer _uniformBuf;
    VkDeviceMemory _vertexBufMem;
    VkDeviceMemory _uniformBufMem;
    /* descriptor */
